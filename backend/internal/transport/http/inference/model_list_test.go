...
func TestAppendReasoningModelAliasesUsesRealSupportedLevels(t *testing.T) {
	...
	base := newModelListItems([]modeldomain.Route{
		...
		{PublicID: "Build/grok-4.7", Provider: account.ProviderBuild, Capability: modeldomain.CapabilityResponses, CreatedAt: now},
	...
	for _, want := range []string{"grok-4.5", "grok-4.5-low", "grok-4.5-medium", "grok-4.5-high", "grok-4.6", "grok-4.6-low", "grok-4.6-medium", "grok-4.6-high", "grok-4.6-xhigh", "grok-4.7", "grok-4.7-low", "grok-4.7-medium", "grok-4.7-high", "grok-4.7-xhigh", "grok-4.3-none", "grok-4.3-low", "grok-4.3-medium", "grok-4.3-high", "grok-build-0.1"} {
		if !ids[want] {
...
func TestAppendReasoningModelAliasesIncludesConsoleGrok47XHigh(t *testing.T) {
	expanded := appendReasoningModelAliases([]modelListItem{{
		ID: "grok-4.7", Provider: account.ProviderConsole, Capability: modeldomain.CapabilityResponses,
	}})
	ids := make(map[string]bool, len(expanded))
	for _, item := range expanded {
		ids[item.ID] = true
	}
	for _, want := range []string{"grok-4.7-low", "grok-4.7-medium", "grok-4.7-high", "grok-4.7-xhigh"} {
		if !ids[want] {
			t.Fatalf("missing Console alias %q in %#v", want, expanded)
		}
	}
