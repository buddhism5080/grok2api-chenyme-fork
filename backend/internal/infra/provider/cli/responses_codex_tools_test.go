...
func TestNormalizeResponsesRequestOmitsEmptyCallIDOrphansByType(t *testing.T) {
	tests := []struct {
		name string
		item string
	}{
		{name: "function_call", item: `{"type":"function_call","id":"fc_1","name":"broken","arguments":"{}"}`},
		{name: "function_call blank call_id", item: `{"type":"function_call","call_id":"   ","name":"broken","arguments":"{}"}`},
		{name: "function_call_output", item: `{"type":"function_call_output","output":"ok"}`},
		{name: "custom_tool_call", item: `{"type":"custom_tool_call","name":"apply_patch","input":"x"}`},
		{name: "apply_patch_call", item: `{"type":"apply_patch_call","operation":{"type":"delete_file","path":"old.txt"}}`},
		{name: "local_shell_call", item: `{"type":"local_shell_call","action":{"type":"exec","command":["pwd"]}}`},
		{name: "tool_search_call", item: `{"type":"tool_search_call","execution":"client","arguments":{"goal":"x"}}`},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
