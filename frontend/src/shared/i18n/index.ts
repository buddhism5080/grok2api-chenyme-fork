    ...
    translation: {
      ...
      settings: {
        ...
        accounts: {
          ...
          buildHighTokenSpeedThresholdHelp: "单位：Token/s。开启功能时默认 1000。仅统计成功的流式请求。禁用侧公式：speed = outputTokens × 1000 / (durationMs − overheadMs)。",
          buildHighTokenSpeedOverheadMS: "速度计算 overhead（ms）",
          buildHighTokenSpeedOverheadMSHelp: "固定从总耗时扣除的延迟预算，默认 2000ms。禁用侧公式：speed = outputTokens × 1000 / (durationMs − overheadMs)。与审计页不同：审计页用 durationMs − firstTokenMs（不扣固定 overhead，缓冲/晚首包时会虚高）。outputTokens < 1000 或 duration ≤ overhead 时不判定。",
