  ...
  "zh-CN": {
    translation: {
      ...
      keys: {
        ...
        modelAliasesBuildSupport: "Grok Build：grok-4.7、grok-4.6 支持 low / medium / high / xhigh；grok-4.5、grok-3-mini、grok-3-mini-fast 支持 low / medium / high。",
        modelAliasesConsoleSupport: "Grok Console：grok-4.7、grok-4.6 支持 low / medium / high / xhigh；grok-4.3 支持 none / low / medium / high；grok-4.20-0309-reasoning 为固定推理模式，不支持调节 effort；grok-4.20-multi-agent-0309 额外支持 xhigh。",
      ...
      audits: {
        ...
        errorLabel: "错误",
        statusMissingReasoning: "无推理惩罚",
        missingReasoningPenalty: "无推理调度惩罚",
        missingReasoningPenaltyApplied: "已施加 24 小时调度惩罚",
      ...
      settings: {
        ...
        audit: {
        ...
        },        clientKeys: { title: "密钥默认限制", rpmLimit: "默认 RPM", rpmLimitHelp: "新建客户端密钥默认允许的每分钟请求数。", maxConcurrent: "默认并发数", maxConcurrentHelp: "新建客户端密钥默认允许同时执行的请求数。" },        accounts: {          title: "账号维护",
          ...
          highTokenSpeedTitle: "高 Token 速度自动禁用",
          buildHighTokenSpeedAutoDisable: "启用",
          buildHighTokenSpeedAutoDisableHelp: "默认关闭。开启后，当 Grok Build 指定模型的输出 Token 速度超过阈值时，自动禁用该账号。过高速度通常意味着上游把请求分流到了低级模型。禁用侧速度算法与请求审计页不同（见下方 overhead 说明）。",
          buildHighTokenSpeedThreshold: "Token 速度阈值",
          buildHighTokenSpeedThresholdHelp: "单位：Token/s。开启功能时默认 1000。仅统计成功的流式请求。禁用侧公式：speed = (outputTokens + reasoningTokens) × 1000 / (durationMs − overheadMs)。",
          buildHighTokenSpeedOverheadMS: "速度计算 overhead（ms）",
          buildHighTokenSpeedOverheadMSHelp: "固定从总耗时扣除的延迟预算，默认 2000ms。禁用侧公式：speed = (outputTokens + reasoningTokens) × 1000 / (durationMs − overheadMs)。与审计页不同：审计页用 durationMs − firstTokenMs（不扣固定 overhead，缓冲/晚首包时会虚高）。(outputTokens + reasoningTokens) < 1000 或 duration ≤ overhead 时不判定。",
          buildHighTokenSpeedModelIDs: "监控模型 ID",
          buildHighTokenSpeedModelIDsHelp: "每行一个公开模型 ID（如 grok-4.20）。留空表示不自动禁用任何账号。最多 64 个。",
          buildHighTokenSpeedModelIDsPlaceholder: "grok-4.20",
          missingReasoningTitle: "无推理输出调度惩罚",
          buildMissingReasoningPenaltyEnabled: "启用",
          buildMissingReasoningPenaltyEnabledHelp: "默认关闭。开启后，Grok Build 指定模型若返回 HTTP 200、输出 token 大于 20 且推理 token 为 0，则对该账号施加 24 小时调度惩罚，并取消当前会话粘滞。",
          buildMissingReasoningPenaltyModelIDs: "监控模型 ID",
          buildMissingReasoningPenaltyModelIDsHelp: "每行一个公开模型 ID（如 grok-4.6）。留空表示不惩罚任何账号。最多 64 个。",
          buildMissingReasoningPenaltyModelIDsPlaceholder: "grok-4.6",
          buildMissingReasoningPenaltyUserTurnModelIDs: "仅用户 prompt 轮惩罚",
          buildMissingReasoningPenaltyUserTurnModelIDsHelp: "对这些模型，仅当请求最后一条输入是用户 prompt 时才施加无推理调度惩罚；若最后一条是工具调用结果则跳过。留空表示所有监控模型都按原规则惩罚。最多 64 个。",
          buildMissingReasoningPenaltyUserTurnModelIDsPlaceholder: "grok-4.7",
        },
        units: { seconds: "秒", minutes: "分", hours: "时", days: "天" },
      },
      settingsBuildTransport: { responseHeaderTimeout: "响应头超时", responseHeaderTimeoutHelp: "请求体发送完成后，等待 Grok Build 返回首个响应头的最长时间。", streamFirstCharTimeout: "流式首字延迟超时", streamFirstCharTimeoutHelp: "从响应头到手到客户端的首个内容 token 的最长时间（5–60 秒）。与审计页「首字」口径一致；超时后断开连接并记失败。", streamFirstCharTimeoutEnabled: "启用流式首字延迟超时", streamFirstCharTimeoutEnabledHelp: "默认关闭。开启后，从响应头到手到客户端的首个内容 token（与审计「首字」一致）的最长时间（5–60 秒）。超时断开流、记失败审计，并按重试机制处理。", streamIdleTimeout: "流式空闲超时", streamIdleTimeoutHelp: "流式响应中连续无有效生成内容或工具进度的最长等待时间；心跳与内部控制事件不会延长等待，超时后主动中断。" },
      egressProxyProfiles: {
        title: "代理地址库",
        libraryTitle: "代理地址库",
      ...
      settingsBuildForbidden: {        markInvalid: "按错误码标记失效",        markInvalidHelp: "启用后，Grok Build 返回 403 且错误码命中下方规则时，将账号标记为失效并移出调度。",        codes: "失效错误码",        codesHelp: "每行填写一个错误码，完整匹配 JSON 响应中的 code 或 error.code（忽略大小写），最多 32 个。",
  ...
  en: {
    translation: {
      ...
      audits: {
        ...
        errorLabel: "Error",
        statusMissingReasoning: "Missing-reasoning penalty",
        missingReasoningPenalty: "Missing-reasoning penalty",
        missingReasoningPenaltyApplied: "24-hour scheduling penalty applied",
