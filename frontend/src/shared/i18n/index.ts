  ...
  "zh-CN": {
    translation: {
      ...
      settings: {
        ...
        routing: { title: "路由策略", stickyTTL: "会话粘性时长", stickyTTLHelp: "同一会话优先复用已选账号的有效时长；到期后重新参与调度。", cooldownBase: "基础冷却时间", cooldownBaseHelp: "账号发生可恢复故障后首次进入冷却的时长。", cooldownMax: "最大冷却时间", cooldownMaxHelp: "连续故障触发指数退避时允许达到的最长冷却时间。", capacityWait: "账号满载等待", capacityWaitHelp: "全部候选账号并发已满时，切换或失败前等待容量释放的时间。", maxAttempts: "最大尝试次数", maxAttemptsHelp: "单次请求在可用账号之间切换并重试的最大次数，范围为 1–200。", preferFreeBuild: "Grok Build Free 账号优先", preferFreeBuildHelp: "启用后优先选择已确认可用的 Free 账号，其他路由条件保持不变。", accountIsolatedConnections: "按账号隔离上游连接", accountIsolatedConnectionsHelp: "开启后，不同账号使用各自的上游 TCP/HTTP 连接池，便于外部 L4 或按连接哈希的负载均衡器分散流量；同一账号仍可复用连接。适用于 Build / Web / Console 出站及 Web 转 Build。会增加连接数、TLS 握手、内存和文件描述符占用，建议仅在确有外部连接级负载均衡需求时开启。", buildHighTokenSpeedAutoDisable: "Build 异常高速度自动禁用", buildHighTokenSpeedAutoDisableHelp: "默认关闭。开启后，当 Grok Build 指定模型的输出 Token 速度超过阈值时，自动禁用该账号。禁用侧速度算法与请求审计页不同。", buildHighTokenSpeedThreshold: "Token 速度阈值", buildHighTokenSpeedThresholdHelp: "单位：Token/s。默认 1000。公式：speed = (outputTokens + reasoningTokens) × 1000 / (durationMs − overheadMs)。", buildHighTokenSpeedOverheadMS: "速度计算 overhead（ms）", buildHighTokenSpeedOverheadMSHelp: "默认 2000。speed = (outputTokens + reasoningTokens) × 1000 / (durationMs − overheadMs)；审计页用 durationMs − firstTokenMs。", buildHighTokenSpeedModelIDs: "监控模型 ID", buildHighTokenSpeedModelIDsHelp: "每行一个公开模型 ID（如 grok-4.20）。留空表示不自动禁用任何账号。最多 64 个。", buildHighTokenSpeedModelIDsPlaceholder: "grok-4.20" },
      ...
      settingsBuildTransport: { responseHeaderTimeout: "响应头超时", responseHeaderTimeoutHelp: "请求体发送完成后，等待 Grok Build 返回首个响应头的最长时间。", streamFirstCharTimeoutEnabled: "启用流式首字延迟超时", streamFirstCharTimeoutEnabledHelp: "默认关闭。开启后，从响应头到手到客户端的首个内容 token（与审计「首字」一致）的最长时间（5–60 秒）。超时断开流、记失败审计，并按重试机制处理。", streamFirstCharTimeout: "流式首字延迟超时", streamFirstCharTimeoutHelp: "从响应头到手到客户端的首个内容 token 的最长时间（5–60 秒）。与审计页「首字」口径一致；超时后断开连接并记失败。", streamIdleTimeout: "流式空闲超时", streamIdleTimeoutHelp: "流式响应中连续无新数据到达的最长等待时间，超时后主动中断。" },
