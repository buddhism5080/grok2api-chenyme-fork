        ...
        <Tabs defaultValue="build" className="flex min-w-0 flex-col gap-7 lg:flex-row lg:items-start">
          ...
          >
            <div className="space-y-0">
              ...
              <SettingsField controlId="provider-stream-first-char-timeout-enabled" label={t("settingsBuildTransport.streamFirstCharTimeoutEnabled")} description={t("settingsBuildTransport.streamFirstCharTimeoutEnabledHelp")}>
                <Controller control={form.control} name="providerBuild.streamFirstCharTimeoutEnabled" render={({ field }) => <div className="flex h-9 items-center"><Switch id="provider-stream-first-char-timeout-enabled" checked={field.value} onCheckedChange={field.onChange} /></div>} />
              </SettingsField>
              <SettingsField controlId="provider-stream-first-char-timeout" label={t("settingsBuildTransport.streamFirstCharTimeout")} description={t("settingsBuildTransport.streamFirstCharTimeoutHelp")} error={form.formState.errors.providerBuild?.streamFirstCharTimeout?.message}>
                <Controller control={form.control} name="providerBuild.streamFirstCharTimeout" render={({ field }) => <DurationInput id="provider-stream-first-char-timeout" disabled={!form.watch("providerBuild.streamFirstCharTimeoutEnabled")} value={field.value} onChange={field.onChange} />} />
              </SettingsField>
          ...
          <SettingsSection title={t("settings.routing.title")}>
            <div className="space-y-0">
              ...
              <SettingsField controlId="routing-build-usage-penalty-token-threshold" label={t("settings.routing.buildUsagePenaltyTokenThreshold")} description={t("settings.routing.buildUsagePenaltyTokenThresholdHelp")} error={form.formState.errors.routing?.buildUsagePenaltyTokenThreshold?.message}><Input id="routing-build-usage-penalty-token-threshold" type="number" min={0} max={1_000_000_000_000} {...form.register("routing.buildUsagePenaltyTokenThreshold", { valueAsNumber: true })} /></SettingsField>
          ...
          <SettingsPane value="accounts">
            ...
            <SettingsSection title={t("settings.accounts.highTokenSpeedTitle")}>
              <div className="space-y-0">
                <SettingsField controlId="accounts-build-high-token-speed-auto-disable" label={t("settings.accounts.buildHighTokenSpeedAutoDisable")} description={t("settings.accounts.buildHighTokenSpeedAutoDisableHelp")}>
                  <Controller control={form.control} name="routing.buildHighTokenSpeedAutoDisable" render={({ field }) => (
                    <div className="flex h-9 items-center">
                      <Switch id="accounts-build-high-token-speed-auto-disable" checked={field.value} onCheckedChange={field.onChange} />
                    </div>
                  )} />
                </SettingsField>

                <SettingsField controlId="accounts-build-high-token-speed-threshold" label={t("settings.accounts.buildHighTokenSpeedThreshold")} description={t("settings.accounts.buildHighTokenSpeedThresholdHelp")} error={form.formState.errors.routing?.buildHighTokenSpeedThreshold?.message}>
